...
//funcao atualizar carrossel

function atualizarCarrossel(){
  divCarrossel.innerHTML ="";//Limpa o carrossel

  imagensUrl.forEach(imagem =>{
    let img=document.createElement("img");//criando uma tag<html>
  ...
  divCarrossel.style.width = `${imagensUrl.length*100}%`//ajusta a largura do carrosel
//iniciar o carrossel
iniciarCarrossel();
}
//funçao para iniciar carrossel
function iniciarCarrossel(){
    ...
    setInterval(()=>{
        ...
        divCarrossel.style.transition=`translateX(-${index*(100/imagensUrl.length)}%)`//move corretamente
    },2000);
}
