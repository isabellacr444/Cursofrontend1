...
import { deleteUsuario, getUsuariosById, uptateUsuario } from "@/controllers/UsuarioController";
...
export async function GET({params}:{params:Parametro}) {
    try {
        ...
        const data = await getUsuariosById(id); //busca o usuario pelo id
...
export async function PATCH({ params }: { params: Parametro }, req: NextRequest) {
  try {
    ...
    const usuarioAtualizado = await uptateUsuario(id, data); //busca o usuario pelo id
    if (!data) {
